(function () {
    ...
    function ($scope, $location, $rootScope, $controller, dataService, userData) {
        ...
        $scope.filterIncludeColumns = ["key", "data", "module_name", "comment"];

        $scope.active = function () {
            if ($scope.loadingData == false) {
                $scope.loadingData = true;
                dataService.get("./api/admin/settings?pagesize=" + $scope.pageSize + "&currentpage=" + $scope.currentPage + "&filter=" + $scope.getFilter()).then(function (response) {
